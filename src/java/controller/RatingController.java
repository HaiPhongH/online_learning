...
public class RatingController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        RatingDAO rb = new RatingDAO();
        DatabaseDAO db = new DatabaseDAO();
        int timesRating = rb.getTimesRating(courseID);
        Course course = db.getCourseById(courseID);
        float rating = course.getRating();
        float newRating = (rating*timesRating+star)/(timesRating+1);
        if(rb.updateTimesRating(courseID)) {
            rb.updateRating(courseID, newRating);
        }
    }
